...
fun renderProductTable(): String {
    return html {
        table {
            ...
            getProducts().mapIndexed { i, p ->
                tr {
                    td(color = getCellColor(i, 0)) { text(p.description) }
                    td(color = getCellColor(i, 0)) { text(p.price) }
                    td(color = getCellColor(i, 0)) { text(p.popularity) }
                }
            }
        }
    }.toString()
